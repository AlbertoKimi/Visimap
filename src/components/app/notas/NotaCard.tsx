...
export const NotaCard: React.FC<NotaCardProps> = ({ nota, onNotaUpdated, onEditClick, showToast }) => {
  ...
  // ¿Hay al menos un botón de acción visible? 
  const puedeEditar = isCreator && nota.estado === 'normal';
  const puedePendiente = nota.estado === 'normal' && hasTogglePermission;
  const puedeFinalizar = (nota.estado === 'normal' || nota.estado === 'pendiente') && hasTogglePermission;
  const hayAcciones = puedeEditar || puedePendiente || puedeFinalizar || hasDeletePermission;
  ...
  return (
    <div
      className={`p-5 rounded-sm shadow-md border flex flex-col h-full transition-all duration-300 relative overflow-hidden transform hover:-translate-y-1 hover:scale-[1.02] hover:shadow-lg active:scale-[0.98] sm:active:scale-[0.99] origin-center ${isDeleting ? 'animate-delete' : ''
        } ${getColorClasses()}`}
    >
      ...
      <div className="flex justify-between items-start gap-2 mb-2 mt-2">
        <span className="inline-block min-w-0 max-w-full text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 bg-white/40 dark:bg-slate-800/40 px-1.5 py-0.5 rounded break-words">
          {asignadoTexto}
        </span>

        {hayAcciones && (
          <div className="flex gap-0.5 shrink-0 bg-white/50 dark:bg-slate-800/50 rounded-lg p-1 items-center">
      ...
      <h3 className="font-semibold text-lg text-slate-800 dark:text-slate-100 break-words leading-tight mb-2">
        {nota.titulo}
      </h3>
