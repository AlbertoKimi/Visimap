...
export const GraficosPanel: React.FC = () => {
  ...
  const fetchEspania = useCallback(async () => {
    setLoadingEspania(true);
    try {
      const { inicio, fin } = mesActualRango();
      ...
      setDatosEspania([{ name: getNombreMesActual(), España: españa, 'Resto del Mundo': mundo }]);
    } catch (err) {
  ...
  const fetchEventosMes = useCallback(async () => {
  ...
  const fetchTodo = useCallback(() => {
    ...
    fetchTrabajadores();
    fetchEventosMes();
  }, [fetchEspania, fetchEvolucion, fetchProvincias, fetchTrabajadores, fetchEventosMes]);

  useEffect(() => {
    fetchTodo();
    const intervalo = setInterval(fetchTodo, 60_000);
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 w-full pb-10">
      ...
      {/* 1. Evolución diaria (Area) - Ancho completo o primero */}
      ...
      {/* 2. Provincias (Bar horizontal con scroll manual si crece) */}
      <TarjetaGrafico
